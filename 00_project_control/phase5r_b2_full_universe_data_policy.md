...
- Under the existing daily wrapper, a normal Massive fetch is attempted only in the unchanged 17:45 ET slot of a regular U.S. market-session day. Earlier weekday slots, weekends, and the final daily-decision refresh must use the bounded `--reuse-validated-snapshot` path instead.
- The scheduler durably reserves that one post-close attempt before starting the child. A crash, timeout, provider failure, or later deterministic-step failure cannot cause another Massive request on a subsequent 15-minute tick that day.
...
- Scoring independently compares each candidate row with the latest completed market session and emits `insufficient_data` rather than an actionable-looking score for a preserved stale row.
