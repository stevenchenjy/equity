...
def validate_annotation_set(
    *,
    ...
    if (
        ...
        or payload["rubric_version"] != REFERENCE_RUBRIC_VERSION
    ...
    for index, item in enumerate(records):
        ...
        if (
            ...
            or record["reference_classification"] not in TRANSITION_CLASSIFICATIONS
        ...
        consensus_rationale = _sha256(
            record["consensus_rationale_sha256"],
            label=f"annotation record {index} consensus rationale hash",
        )
        pair = (
        ...
        for reviewer_index, value in enumerate(attestations):
            ...
            if (
                attestation["is_material_transition"]
                is not record["is_material_transition"]
                or attestation["reference_classification"]
                != record["reference_classification"]
                or attestation["reference_thesis_direction"]
                != record["reference_thesis_direction"]
            ):
                raise AnnotationError("independent reviewers did not agree")
