...
REFRESH_PIPELINE = ROOT / "09_scripts" / "equity_research" / "run_daily_refresh.py"
SEC_EVIDENCE_REFRESH = ROOT / "09_scripts" / "equity_research" / "refresh_daily_evidence.py"
MASSIVE_B2_RUNNER = (
...
def _run_market_discovery_only() -> int:
    try:
        completed = subprocess.run(
            [sys.executable, str(ROOT / "09_scripts/equity_research/market_discovery.py"), "--refresh"],
            cwd=ROOT,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
...
def main() -> int:
    ...
    from news_schedule import run_due_news_checks
    try:
