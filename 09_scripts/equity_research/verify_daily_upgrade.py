...
from send_daily_email import cycle_is_blocked, delivery_policy


SCHEDULER_DIR = ROOT / "07_automation" / "scheduler"
CONTROL_REPORT = ROOT / "00_project_control" / "daily_verification_report.md"
RESEARCH_REPORT = (
    ROOT
    / "04_research"
    / "company_research"
    / "daily_verification_report.md"
)
VERIFICATION_LOG = (
    ROOT / "00_project_control" / "run_logs" / "daily_verification_log.csv"
)
...
PRODUCTION_RUNTIME_WRAPPER = (
    ...
    / "09_scripts"
    / "equity_research"
    / "run_runtime_scheduler.py"
)
EXTERNAL_DAILYREFRESH_LAUNCHER = Path(
    "/Users/messssi/Library/Application Support/EquityResearch/bin/"
    "dailyrefresh_launcher.py"
)
LEGACY_LABELS = (
...
NEW_JOBS = {
    "com.steven.equity_research.dailyrefresh": "dailyrefresh",
    "com.steven.equity_research.dailydecision": "dailydecision",
}
NEW_PYTHON_FILES = (
    "active_config.py",
    "daily_common.py",
...
def source_checks(checks: list[dict[str, str]]) -> None:
    ...
    refresh_source = (script_dir / "run_daily_refresh.py").read_text(
        encoding="utf-8"
    )
    add_check(
        checks,
        ...
        all(
            ...
            for token in ("send_daily_email", "smtplib", "EMAIL_CONFIG_PATH")
        ),
    ...
    sender_source = (script_dir / "send_daily_email.py").read_text(
        encoding="utf-8"
    )
    ...
    add_check(
        checks,
        "smtp.single_owner",
        sorted(set(config_openers)) == ["send_daily_email.py"],
    ...
    retired_active_paths = (
        script_dir / "send_c2_daily_email.py",
        script_dir / "run_c3_daily_email_pipeline.py",
        script_dir / "run_llm_shadow.py",
        script_dir / "run_production_shadow.py",
    )
