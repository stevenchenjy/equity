...
from test_shadow_llm import fake_packet, valid_analyst, valid_critic, valid_judge
from shadow_llm_contract import (
    build_automatic_evaluation, build_blind_judge_target,
    build_deterministic_baseline, deterministic_claim_check,
)
from evaluate_shadow_llm_incremental_value import (
    _deduplicate_evidence, _official_follow_up, aggregate,
)
from run_shadow_llm_evaluation import load_config
...
class ShadowMeasurementTests(unittest.TestCase):
    def test_latest_official_packet_supplements_historical_proofs_and_is_wired(self) -> None:
        ...
        with (patch("evaluate_shadow_llm_incremental_value._packet_for_bundle", return_value=archived),
              patch("evaluate_shadow_llm_incremental_value._official_follow_up", wraps=_official_follow_up) as follow):
        ...
        wrapper = SCRIPT_DIR.parents[1] / "07_automation/scheduler/run_shadow_llm_event.sh"
    ...
    def test_aggregate_deduplicates_substantive_coverage_and_labels_recall(self) -> None:
        ...
        with patch("evaluate_shadow_llm_incremental_value._packet_for_bundle", return_value=fake_packet()):
    ...
    def test_different_offsets_in_same_documents_do_not_inflate_event_count(self) -> None:
        ...
        bundles = [evaluation_bundle("one"), evaluation_bundle("two")]
        original = copy.deepcopy(bundles)
        with patch("evaluate_shadow_llm_incremental_value._packet_for_bundle", side_effect=[first, second]):
            result = aggregate(bundles, load_config(), snapshot_path=Path("/missing/snapshots"), outcome_path=Path("/missing/outcomes"))
    ...
    def test_missing_sealed_packet_cannot_claim_baseline_incremental_value(self) -> None:
        with patch("evaluate_shadow_llm_incremental_value._packet_for_bundle", return_value=None):
