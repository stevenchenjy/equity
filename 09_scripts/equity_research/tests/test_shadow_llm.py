...
from shadow_llm_provider import (
    CodexCliProvider,
    ShadowProviderError,
    classify_nonzero_exit,
    cli_reported_token_usage,
)
import run_shadow_llm_evaluation as shadow_runner
...
class ShadowLlmTests(unittest.TestCase):
    def test_evaluation_scheduler_is_event_driven_and_nonproduction(self) -> None:
        ...
        template = scheduler_dir / "com.steven.equity_research.shadoweval.plist.template"
        with template.open("rb") as handle:
            payload = plistlib.load(handle)
        self.assertEqual(payload["Label"], "com.steven.equity_research.shadoweval")
        self.assertFalse(payload["RunAtLoad"])
        self.assertNotIn("StartInterval", payload)
        self.assertEqual(
            payload["WatchPaths"],
            [
                "/Users/messssi/LocalRuntime/equity/03_source_data/equity_research/"
        ...
        wrapper = (scheduler_dir / "run_shadow_llm_event.sh").read_text(
            encoding="utf-8"
        )
        for prohibited in ("send_daily_email", "smtp", "broker", "order"):
    ...
    def test_production_entrypoints_do_not_reference_shadow_surface(self) -> None:
        ...
        prohibited = (
            "run_shadow_llm_evaluation",
            "shadow_llm_provider",
            "evaluate_shadow_llm_incremental_value",
        )
    ...
    def test_cli_provider_uses_json_usage_minimal_env_and_disabled_tools(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch(
                    "shadow_llm_provider.executable_sha256",
                    return_value="d" * 64,
                ),
                patch(
                    "shadow_llm_provider.subprocess.run", side_effect=fake_run
                ),
            ):
