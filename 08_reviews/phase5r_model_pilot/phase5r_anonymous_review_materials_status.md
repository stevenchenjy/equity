...
- five blinded committee/critic rows for incremental-value review;
- an immutable blank review template and a separately sealed model key; and
- exact per-call token/cost receipts plus a no-go-pending-review report.

Reviewers must complete separate copies of the anonymous review file. The
immutable generated original, sealed key, and assignment file must not be
distributed together. See
`08_reviews/phase5r_model_pilot/phase5r_anonymous_review_protocol.md`.

Pre-inference evidence available for audit:

- 10 real-source point-in-time packet files;
- 4 chronological transition probes;
- 5 adversarial safety probes;
- 6 verified raw issuer-submission snapshots;
- 4 verified Company Facts snapshots and accession reconciliations;
- 6 hash-bound exhibit-discovery manifests and 5 downloaded `EX-*` files;
